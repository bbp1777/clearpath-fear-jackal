...
class MANN_Handler():
    def __init__(self,file_path,batch_size,look_back=3):
        ...
        self.model = EncapsulatedNTM(1600 + 2, 2,1,controller_size=250, controller_layers=5, 
    ...
    def load_data_get_vals(self,file_path):
        data_obs = np.load(file_path+"CarRacing-v2_lookback_3observationsfinal_shapebw_shape.npy")
        data_class = np.load(file_path+"CarRacing-v2_lookback_3class.npy")
        data_class_num = np.load(file_path+"CarRacing-v2_lookback_3class_number.npy")
    ...
    def train_model(self):
        ...
        for epoch in range(0, int(300)): #300
            ...
            print(float((100.0*episode_correct)/(self.batch_size*self.batches)))
        self.model=model
    ...
    def calculate_reward(self,img,reward_type):
        with torch.no_grad():
