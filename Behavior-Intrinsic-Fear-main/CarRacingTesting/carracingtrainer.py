# -*- coding: utf-8 -*-
"""
Created on Mon Jan 26 17:01:11 2026

@author: rodne
"""

from ppoagenthandler import PPO as Agent
from car_racing_wrapper import POMDPNDTCWrapper as EnvHandler
from loggerutils import TLogger
from dataclasses import dataclass, field
from train_and_set_model import MANN_Handler
from train_and_set_model_complex_car_racing import MANN_Handler as MANN_HandlerComplex 
...
class LoggerArgs:
    ...
    """experiment name"""
    controller_type:str ="_complex_controller"
    """descrives the controller used in the experiment"""
    test_condition:str="Testing"
...
class RunArgs:
    ...
    """ sets the threshhold of accuracy where the intrinsic reward will begin to kick in """
    intrisinc_only:bool =False
    """ sets if intrinsic value is set to true """
    intrinsic_reward_type:str="positive"
    """ variable to deliniate positive only, negative only, or both"""


class carracingtraininer():
    ...
    def __init__(self,iterator,thresh_index=None,RunArgs=RunArgs,AgentArgs=AgentArgs,
        ...
        if not self.run_params.intrisic_run:
            self.logger_params.save_folder=self.logger_params.save_folder+self.logger_params.exp_name[iterator]+"no_intrinsic"+self.env_params.env_name
            
        elif self.run_params.which_mann=="normal":
            self.logger_params.save_folder=self.logger_params.save_folder+self.logger_params.exp_name[iterator]+self.run_params.which_mann+self.run_params.intrinsic_type+self.env_params.env_name
        elif self.run_params.which_mann=="complex":
            self.logger_params.save_folder=self.logger_params.save_folder+self.logger_params.exp_name[iterator]+self.run_params.which_mann+self.run_params.intrinsic_type+str(self.run_params.threshold[self.thresh_index])+f"intrisinc_only is {self.run_params.intrisinc_only}"+self.env_params.env_name
    ...
    def _set_intrinsic(self):
        if self.run_params.intrisic_run ==True:
            if self.run_params.which_mann == "normal":
                ...
                self.handler=MANN_Handler("Data//CarRacing_2_Steps_Class//",2)
                ...
                self.handler=MANN_HandlerComplex("Data//CarRacing_2_Steps_Class//",2)
    ...
    def _get_intrinsic(self,global_step,state,intrisic_type):
        if self.run_params.intrisic_run ==True:
            ...
            if self.run_params.which_mann=="normal":
                if global_step >0:
                    ...
                    intrinsic_reward=-1*(self.handler.calculate_reward(state_mann,reward_type=intrisic_type))
            else:
                if global_step >2:
    ...
    def _get_intrinsic_thresholded(self,global_step,state,intrisic_type):
        if self.run_params.intrisic_run ==True:
            self.mann_states.append(state)
            if self.run_params.which_mann=="normal":
                if global_step >0:
                    ...
                    intrinsic_reward=-1*(self.handler.calculate_threshholded_fear(state_mann,reward_type=intrisic_type))
            else:
                if global_step >2:
                    state_mann=np.asarray(list(self.mann_states))
    ...
    def choose_intrinsic(self,global_step,state):
        return self.dictionay_intrinsic[self.intrinsic_type](global_step,state,self.run_params.intrinsic_reward_type)
    ...
    def run_test(self):
        ...
        for ep in range(self.run_params.episodes):
            ...
            while  (done==False or truncated==False):
                ...
                if done==True:
                    print("print its in the grass")
                    plt.imshow(state.reshape(84,84,3))
                    plt.show(block=True)   
                ...
                if self.run_params.intrisinc_only:
                    joint_reward=intrinsic_reward
                else:                    
...
if __name__ == "__main__":
    ...
    if run_args.which_mann=="normal":
        thresh=None
        for i in range(5):
            trainer=carracingtraininer(i,thresh)
            trainer()
    else:
