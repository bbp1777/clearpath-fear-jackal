...
class miniworltraininer():
    ...
    def __init__(self,iterator,thresh_index=None,RunArgs=RunArgs,AgentArgs=AgentArgs,
        ...
        if not self.run_params.intrisic_run:
            ...
            self.logger_params.save_folder=self.logger_params.save_folder+self.logger_params.exp_name[iterator]+self.run_params.which_mann+self.run_params.intrinsic_type+str(self.run_params.threshold[self.thresh_index])+f"intrisinc_only is {self.run_params.intrisinc_only}"
