...
class FearTrainer(Node):
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter('intrinsic_reward_scale', 0.25)
        ...
        self.declare_parameter('goal_progress_scale', 5.0)
        self.declare_parameter('goal_alignment_scale', 0.5)
        self.declare_parameter('step_penalty', -0.01)
        self.declare_parameter('collision_penalty', -1.0)
        self.declare_parameter('goal_reward_bonus', 2.0)
        ...
        # This flag disables all online learning so live runs stay frozen for evaluation.
        self.declare_parameter('evaluation_only', True)
    ...
    def _log_runtime_mode(self) -> None:
        ...
        if self.trainer_config.evaluation_only:
            self.get_logger().info(
                ...
                f'fear_model_mode={self.agent_config.fear_model_mode} '
        ...
        self.get_logger().info('Online learning mode is enabled; PPO and SMANN updates may change the policy during the run.')
    ...
    def _record_reward_step(
        ...
        if self.writer is not None:
            self.writer.add_scalar('train/external_reward_step', float(external_reward), self.global_step)
            self.writer.add_scalar('train/intrinsic_reward_step', float(intrinsic_reward), self.global_step)
            self.writer.add_scalar('train/combined_reward_step', float(self.agent.last_combined_reward), self.global_step)
            self.writer.add_scalar('train/goal_coverage', float(state.goal_coverage), self.global_step)
            self.writer.add_scalar('train/collision_flag', float(state.collision), self.global_step)
            self.writer.add_scalar('train/fear_score', float(self.agent.last_fear_score), self.global_step)
            self.writer.add_scalar('train/fear_override', float(self.agent.last_fear_override), self.global_step)
    ...
    def _handle_terminal_observation(self, state) -> None:
        ...
        if state.collision:
            reason = 'collision'
        elif state.goal_coverage >= self.environment_config.goal_completion_threshold:
            reason = 'goal'
        else:
            reason = 'truncation'
    ...
    def _finish_episode(self, final_state) -> None:
        ...
        if self.writer is not None:
            self.writer.add_scalar('episode/external_reward', self.episode_external_reward, self.episode_index)
            self.writer.add_scalar('episode/intrinsic_reward', self.episode_intrinsic_reward, self.episode_index)
            self.writer.add_scalar('episode/combined_reward', self.episode_combined_reward, self.episode_index)
        ...
        # This guard keeps the SMANN checkpoint frozen during evaluation sweeps.
        if (not self.trainer_config.evaluation_only) and self.episode_index % self.trainer_config.vicarious_update_every_n_episodes == 0:
    ...
    def _relaunch_managed_simulation(self) -> None:
        ...
        if self._start_managed_sim_process():
            self.get_logger().info(
        ...
        self.get_logger().warning(
            'Failed to fully relaunch the managed simulation process; '
            'trainer will keep waiting for the simulator to come back.'
        )
    ...
    def _start_managed_sim_process(self) -> bool:
        """
        ...
        try:
            ...
            self.get_logger().warning(
                'Unable to start the managed simulation process because bash is unavailable.'
            )
            self._sim_process = None
            return False
        except Exception as exc:
            self.get_logger().warning(
                f'Unable to start the managed simulation process: {exc}'
            )
            self._sim_process = None
            return False
        ...
        if process.poll() is not None:
            self.get_logger().warning(
                f'Managed simulation process exited immediately with code {process.returncode}.'
            )
            self._sim_process = None
            return False
        return True
